...
function loadServer(serverNum) {
  ...
  switch(serverNum) {
    ...
    case "3":
      ...
      break;
    case "4":
      videoSrc = `https://vidup.to/movie/${currentMovieID}`;
      break;
    case "5":
      videoSrc = `https://cinemaos.tech/player/${currentMovieID}`;
      break;
    default:
      return;
  }
...
// --- INFO PAGE — FIXED Play Button Layout ---
async function openInfoPage(item) {
  ...
  try {
    ...
    if(type === "movie") {
      ...
      html += `</div><h3 style="margin:25px 0 15px; font-size:22px;">Episodes</h3>`;
...
function playMovie(tmdbID) {
  ...
  document.getElementById("videoFrame").src = `https://player.videasy.to/movie/${tmdbID}`;
  infoPage.style.display = "none";
...
function playEpisode(tvId, season, ep) {
  if(!tvId || !season || !ep) return;
  currentMovieID = `${tvId}-${season}-${ep}`;
  document.getElementById("videoFrame").src = `https://player.videasy.to/tv/${tvId}/${season}/${ep}`;
  infoPage.style.display = "none";
  playerModal.style.display = "flex";
  document.body.style.overflow = "hidden";
}
